...
@section('title', 'Edit Tugas Proyek')

@section('content')
<div class="max-w-xl mx-auto bg-[#232329] rounded-lg shadow p-8 mt-8">
    <h1 class="text-2xl font-bold mb-8 text-[#38d4ae]">Edit Tugas Proyek</h1>
    @if($errors->any())
    ...
    <form method="POST" action="{{ route('admin.projects.tasks.update', [$project, $task]) }}">
        ...
        <div class="mb-5">
            ...
            <input type="text" name="name" id="name" class="w-full px-3 py-2 rounded border bg-[#18181b] text-[#f3f4f6] focus:ring-2 focus:ring-[#38d4ae] @error('name') border-red-500 @else border-[#38383f] @enderror" value="{{ old('name', $task->name) }}" required autofocus>
            @error('name')
        ...
        <div class="mb-5">
            ...
            <textarea name="description" id="description" class="w-full px-3 py-2 rounded border bg-[#18181b] text-[#f3f4f6] focus:ring-2 focus:ring-[#38d4ae] @error('description') border-red-500 @else border-[#38383f] @enderror" rows="3" required>{{ old('description', $task->description) }}</textarea>
            @error('description')
                <div class="text-red-400 text-xs mt-1">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-8">
            ...
            <input type="date" name="due_date" id="due_date" class="w-full px-3 py-2 rounded border bg-[#18181b] text-[#f3f4f6] focus:ring-2 focus:ring-[#38d4ae] @error('due_date') border-red-500 @else border-[#38383f] @enderror" value="{{ old('due_date', $task->due_date->format('Y-m-d')) }}" required>
            @error('due_date')
                <div class="text-red-400 text-xs mt-1">{{ $message }}</div>
            @enderror
        </div>
