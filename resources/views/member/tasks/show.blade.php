...
<div class="container mx-auto px-4 py-8">
    <div class="bg-[#232329] rounded-lg p-6 shadow border border-[#38383f] mb-6">
        ...
        <form action="{{ route('member.tasks.updateStatus', $task) }}" method="POST" class="mt-4 flex gap-2 items-center">
            @csrf
            @method('PATCH')
            <select name="status" class="px-3 py-2 rounded border border-gray-700 bg-transparent text-[#f3f4f6]">
                ...
                <option value="completed" @if($task->status==='completed') selected @endif>Selesai</option>
            </select>
            <button type="submit" class="px-4 py-2 bg-[#38d4ae] text-[#18181b] rounded hover:bg-[#2bbd99] transition">Update Status</button>
        </form>
    </div>
    <div class="bg-[#232329] rounded-lg p-6 shadow border border-[#38383f]">
        ...
        <form action="{{ route('member.tasks.addComment', $task) }}" method="POST" class="mb-4 flex gap-2">
            @csrf
            <input type="text" name="comment" class="flex-1 px-3 py-2 rounded border border-gray-700 bg-transparent text-[#f3f4f6]" placeholder="Tulis komentar...">
